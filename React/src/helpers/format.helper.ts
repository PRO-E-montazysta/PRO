...
export const formatLocation = (location: Location) => {
    if (!location) return ""
        ...
        result += location.street ? ('ul. ' + location.street + " " + location.propertyNumber +
...
export const formatArrayToOptions = (keyProp: string, formatFn: (value: any) => string, array?: Array<any>): SelectMenuItemProps[] => {
    return array ?
        array.map(x => {
