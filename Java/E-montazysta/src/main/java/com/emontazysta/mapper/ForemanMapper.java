...
public class ForemanMapper {
    ...
    public ForemanDto toDto(Foreman foreman) {
        return ForemanDto.builder()
                ...
                .unavailabilities(foreman.getUnavailabilities().stream()
                        .filter(unavailability -> !unavailability.isDeleted())
                        .map(Unavailability::getId)
                        .collect(Collectors.toList()))
                .notifications(foreman.getNotifications().stream()
                        .filter(notification -> !notification.isDeleted())
                        .map(Notification::getId)
                        .collect(Collectors.toList()))
                .employeeComments(foreman.getEmployeeComments().stream()
                        .filter(comment -> !comment.isDeleted())
                        .map(Comment::getId)
                        .collect(Collectors.toList()))
                .elementEvents(foreman.getElementEvents().stream()
                        .filter(elementEvent -> !elementEvent.isDeleted())
                        .map(ElementEvent::getId)
                        .collect(Collectors.toList()))
                .employments(foreman.getEmployments().stream()
                        .filter(employment -> !employment.isDeleted())
                        .map(Employment::getId)
                        .collect(Collectors.toList()))
                .attachments(foreman.getAttachments().stream()
                        .filter(attachment -> !attachment.isDeleted())
                        .map(Attachment::getId)
                        .collect(Collectors.toList()))
                .toolEvents(foreman.getToolEvents().stream()
                        .filter(toolEvent -> !toolEvent.isDeleted())
                        .map(ToolEvent::getId)
                        .collect(Collectors.toList()))
                .ordersStagesList(foreman.getOrdersStagesList().stream()
                        .filter(orderStage -> !orderStage.isDeleted())
                        .map(OrderStage::getId)
                        .collect(Collectors.toList()))
                .receivedTools(foreman.getReceivedTools().stream()
                        .filter(toolRelease -> !toolRelease.isDeleted())
                        .map(ToolRelease::getId)
                        .collect(Collectors.toList()))
                .assignedOrders(foreman.getAssignedOrders().stream()
                        .filter(orders -> !orders.isDeleted())
                        .map(Orders::getId)
                        .collect(Collectors.toList()))
                .elementReturnReleases(foreman.getElementReturnReleases().stream()
                        .filter(elementReturnRelease -> !elementReturnRelease.isDeleted())
                        .map(ElementReturnRelease::getId)
                        .collect(Collectors.toList()))
                .demandsAdHocs(foreman.getDemandsAdHocs().stream()
